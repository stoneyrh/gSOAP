...
{ int i;  
  ...
  { for (p = Tptr[i]; p; p = p->next)
    { if (!p->generated && !is_transient(p) && p->type != Twchar && !is_XML(p) && !is_void(p))
      { p->generated = True;
        generate(p);
        if (fflag)
          if (--partnum == 0)
...
{ int flag = 0;
  ...
  if (typ->type == Tpointer)
  { if (!is_string(typ) && !is_stdstr(typ->ref))
    { Tnode *ptr = (Tnode*)typ->ref;
      if (ptr->minLength != -1)
        fprintf(fout,"\n\tif (*%s && (*%s)->%s < %ld)\n\t{\tsoap->error = SOAP_LENGTH;\n\t\treturn NULL;\n\t}", obj, obj, name, ptr->minLength);
      if (ptr->maxLength != -1)
        fprintf(fout,"\n\tif (*%s && (*%s)->%s > %ld)\n\t{\tsoap->error = SOAP_LENGTH;\n\t\treturn NULL;\n\t}", obj, obj, name, ptr->maxLength);
    }
  }
...
{ int i, j, d = 0;
  ...
  { for (t = table; t; t = t->prev)
	...
	{ if (p->info.sto & Sattribute)
	    soap_set_attr(p, ptr_cast(t, "a"), ident(p->sym->name), ns_add(p->sym->name, nsa));
        }
    }
  }
  else
