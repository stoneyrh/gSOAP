...
{ Entry *p, *q, *r;
  ...
  { for (p = t->list; p; p = p->next)
	...
	{ if (is_invisible(p->sym->name))
	  ...
	  { fprintf(fd, " <part name=\"parameters\" element=\"%s\"", ns_add(p->sym->name, ns));
	...
	{ q->info.typ->wsdl = True;
	  ...
	  { if (has_ns_eq(NULL, ((Entry*)p->info.typ->ref)->sym->name))
	    ...
	    { fprintf(fd, " <part name=\"parameters\" element=\"%s\"", ns_convert(((Tnode*)q->info.typ->ref)->id->name));
	      if (gen_member_documentation(fd, p->sym, q, ns))
	        fprintf(fd, " </part>\n");
	    }
	  }
...
{ Entry *p, *method, *param;
  ...
  { t = (Table*)p->info.typ->ref;
    ...
    { fprintf(fd, "\n\nvoid %s::soap_header(", name);
      ...
      fprintf(fd, "\n}");
      if (namespaceid)
        fprintf(fd, "\n\nconst SOAP_ENV__Header *%s::soap_header()\n{\treturn (const %s::SOAP_ENV__Header*)%s->header;\n}", name, namespaceid, soap);
      else
        fprintf(fd, "\n\nconst SOAP_ENV__Header *%s::soap_header()\n{\treturn %s->header;\n}", name, soap);
    }
  }
    ...
    { if (aflag)
      ...
      { Entry *param = entry(classtable, method->sym);
        ...
        { param = ((Table*)param->info.typ->ref)->list;
          ...
          { if (action)
              if (*action == '"')
	        fprintf(fd, "\n\tif ((!%s->action && !soap_match_tag(%s, %s->tag, \"%s\")) || (%s->action && !strcmp(%s->action, %s))", soap, soap, soap, ns_convert(param->sym->name), soap, soap, action);
              else
	        fprintf(fd, "\n\tif ((!%s->action && !soap_match_tag(%s, %s->tag, \"%s\")) || (%s->action && !strcmp(%s->action, \"%s\"))", soap, soap, soap, ns_convert(param->sym->name), soap, soap, action);
	    else
              fprintf(fd, "\n\tif (!soap_match_tag(%s, this->tag, \"%s\")", soap, ns_convert(param->sym->name));
      ...
      { if (action)
          if (*action == '"')
	    fprintf(fd, "\n\tif ((!%s->action && !soap_match_tag(%s, %s->tag, \"%s\")) || (%s->action && !strcmp(%s->action, %s))", soap, soap, soap, ns_convert(method->sym->name), soap, soap, action);
          else
	    fprintf(fd, "\n\tif ((!%s->action && !soap_match_tag(%s, %s->tag, \"%s\")) || (%s->action && !strcmp(%s->action, \"%s\"))", soap, soap, soap, ns_convert(method->sym->name), soap, soap, action);
	else
