...
soap_wsrm_request() or soap_wsrm_request_acks() call to set the required WS-RM
information headers for the message send operation or request-response
exchange.
...
soap_wsrm_create_offer(). Because the address may change due to a redirect, we
encourage the use of soap_wsrm_to() for the WS-RM destination address.
...
soap_wsrm_create() or soap_wsrm_create_offer(), then the acknowledgements will
not be returned to the sender (client). In this case message resends are
...
soap_wsrm_retry() function can be used as follows to only retry the message
exchange (or send) when needed:
...
the soap_wsa_add_NoReply() call, which adds a ReplyTo 'noreply' WS-Addressing
header block URI.
...
An error produced by soap_wsrm_sender_fault() or soap_wsrm_receiever_fault()
before soap_wsrm_check() is considered fatal, it will terminate the sequence
and the sender (client) will not be able to continue the sequence
transmissions. While the faults preduced after soap_wsrm_check() allow the
...
opertions should not use soap_wsrm_check() and soap_wsrm_reply(). Because
...
processing capability with the built-in __wsrm__SequenceAcknowledgement()
