...
{ char *s;
  ...
  *seq = NULL;
  ...
  soap_wsa_add_ReplyTo(soap, acksto);
...
{ struct soap_wsrm_data *data;
  ...
  if (soap_wsrm_add_acks(soap, seq))
    return soap->error;
...
soap_wsrm_close(struct soap *soap, soap_wsrm_sequence_handle seq, const char *wsa_id)
{
  ...
  if (soap_wsrm_request(soap, seq, soap_wsa_rand_uuid(soap), SOAP_NAMESPACE_OF_wsrm"/LastMessage"))
    return soap->error;
...
{ struct soap_wsrm_sequence *seq;
  ...
  /* add acks for ackRequested response from server */
  return soap_wsrm_add_acks(soap, seq);
}
...
{ struct soap_wsrm_sequence *seq = NULL;
  ...
  if (soap->header && soap->header->wsrm__AckRequested)
    seq = soap_wsrm_seq_lookup(soap, soap->header->wsrm__AckRequested->Identifier);
  if (seq)
    return soap_wsrm_acknowledgement(soap, seq, soap_wsa_rand_uuid(soap));
  return SOAP_OK;
}
...
{ if (soap->header && soap->header->wsrm__Sequence)
  { int err;
    struct wsrm__SequenceType* Sequence = soap->header->wsrm__Sequence;
    int __sizeAckRequested = soap->header->__sizeAckRequested;
    struct wsrm__AckRequestedType* AckRequested = soap->header->wsrm__AckRequested;
...
/**
@fn int soap_wsrm_add_acks(struct soap *soap, struct soap_wsrm_sequence *seq)
...
*/
static int
soap_wsrm_add_acks(struct soap *soap, struct soap_wsrm_sequence *seq)
{ int numack = 0;
  short oob = 0;
  soap_header(soap);
  if (!soap->header)
  ...
  MUTEX_LOCK(soap_wsrm_session_lock);
  oob = seq->acksto && strcmp(seq->acksto, soap_wsa_anonymousURI) && (!seq->to || strcmp(seq->to, seq->acksto));
/* TODO: consider sending ack for this sequence always
  if (!oob)
    seq->ackreq = 1;
*/
...
@param soap context
@return sequence handle (must be freed with @ref soap_seq_free)
*/
struct soap_wsrm_sequence *
...
@brief Function to check if a sequence is terminated or needs to be
...
{ DBGFUN("soap_wsrm_seq_terminated");
  if (!seq || seq->state == SOAP_WSRM_TERMINATED)
    return 1;
  if (seq->expires && seq->expires < time(NULL))
  { DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Sequence %s has expired\n", seq->id?seq->id:"(null)"));
    seq->state = SOAP_WSRM_TERMINATED;
