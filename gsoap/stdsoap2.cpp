...
#ifdef __cplusplus
SOAP_SOURCE_STAMP("@(#) stdsoap2.cpp ver 2.8.71 2018-11-12 00:00:00 GMT")
extern "C" {
#else
SOAP_SOURCE_STAMP("@(#) stdsoap2.c ver 2.8.71 2018-11-12 00:00:00 GMT")
#endif
...
/* Suggested alternative approach to check the key file for cert only when cafile==NULL */
  ...
  /* disable SSL v3 by default if no SSL/TLS flags are set */
...
#endif
#ifdef WITH_SYSTEMSSL
  gsk_iocallback local_io = {
    ssl_recv, ssl_send, NULL, NULL, NULL, NULL };
  int err, s;
  int retries = 0;
  if (soap->accept_timeout)
  {
    ...
    if (s && httpcmd)
    {
      ...
      if (httpcmd > 1)
      {
        ...
        switch (httpcmd)
        {
          ...
          case  3: soap->error = soap->fpatch(soap); break;
          case  4: soap->error = soap->fget(soap);   break;
          case  5: soap->error = soap->fdel(soap);   break;
          case  6: soap->error = soap->fopt(soap);   break;
          case  7: soap->error = soap->fhead(soap);  break;
...
soap_get_http_body(struct soap *soap, size_t *len)
{
  ...
  if (k && !(soap->mode & SOAP_ENC_ZLIB))
  {
    ...
    if ((SOAP_MAXALLOCSIZE != 0 && k > SOAP_MAXALLOCSIZE) || k > (ULONG64)((size_t)-2))
    {
      soap->error = SOAP_EOM;
      return NULL;
    }
    s = t = (char*)soap_malloc(soap, (size_t)k + 1);
    if (s)
    {
      size_t i;
      for (i = 0; i < k; i++)
