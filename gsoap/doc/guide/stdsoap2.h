...
@see `#SOAP_XML_DOM` and the gSOAP XML DOM API documentation.
*/
#define WITH_DOM

/// When this macro is defined at compile time (undefined by default), replaces UTF-8 content that is outside the Unicode range with the value of `#SOAP_UNKNOWN_UNICODE_CHAR` (Unicode FFFD) to indicate illegal UTF-8, set `#SOAP_UNKNOWN_UNICODE_CHAR` to another integer code if desired (0xFFFD by default)
/**
...
/// User-definable size of the input and output message buffer `::soap::buf` (65536 by default)
/**
...
/// User-definable maximum length of HTTP headers (8192 by default)
/**
@warning When the value of this macro is assigned at the compiler's command line or in an IDE or by specifying `#SOAPDEFS_H`, then all source code files of an application's project must be recompiled.  Otherwise, `::soap` context corruption errors may occur in the non-recompiled parts of the application, because the size of the `::soap` context has changed.
*/
#define SOAP_HDRLEN (8192)

/// User-definable maximum length of XML tags and URLs (1024 by default)
/**
@warning When the value of this macro is assigned at the compiler's command line or in an IDE or by specifying `#SOAPDEFS_H`, then all source code files of an application's project must be recompiled.  Otherwise, `::soap` context corruption errors may occur in the non-recompiled parts of the application, because the size of the `::soap` context has changed.
*/
...
/// User-definable maximum size of a block of memory that `malloc` can allocate or 0 for no limit (0 by default)
...
/**
Deserialized SOAP arrays larger in size than `#SOAP_MAXARRAYSIZE` are not pre-allocated in memory, but deserialized on an element-by-element basis until XML validation contrains kick in (100000 by default)
*/
#define SOAP_MAXARRAYSIZE (100000)

/// User-definable maximum length of DIME attachments received (8 MB by default)
/**
...
/// User-definable maximum number of EINTR interrupts to ignore while polling a socket for pending activity, each EINTR ignored may increase the I/O blocking time by at most one second (10 by default)
...
/// User-definable maximum iterations in the server-side `::soap_serve` loop (or the C++ service class `serve` method) on HTTP keep-alive connections, assigned to `::soap::max_keep_alive` which can be altered at runtime (100 by default)
#define SOAP_MAXKEEPALIVE (100)

/// User-definable maximum string content length for strings not already constrained by XML schema validation constraints, zero or negative means unlimited string lengths are allowed (0 by default)
#define SOAP_MAXLENGTH (0)

/// User-definable maximum XML nesting depth level permitted by the XML parser, must be greater than zero (10000 by default)
#define SOAP_MAXLEVEL (10000)

/// User-definable maximum number of array or container elements for containers that are not already constrained by XML schema validation constraints, must be greater than zero (100000 by default)
...
/// User-definable length of RSA keys used for https connections (2048 by default)
#define SOAP_SSL_RSA_BITS (2048)

/// User-definable 8 bit integer that represents a character that could not be converted to an ASCII char, i.e. when converting an XML Unicode character (typically UTF-8), this is applicable when the runtime flag `#SOAP_C_UTFSTRING` is not used (0x7F by default)
#define SOAP_UNKNOWN_CHAR (0x7F)

/// A user-definable integer Unicode value representing a character that replaces an invalid Unicode code point, i.e. when converting an XML invalid Unicode character from UTF-8 (0xFFFD by default)
...
{
  const struct soap_dom_element *elt = soap->dom;
  ... // inspect the DOM using the DOM API functions (see the gSOAP XML DOM API documentation)
}
~~~
*/
#define SOAP_XML_DOM
...
@see `#SOAP_DOM_NODE`, `#SOAP_DOM_TREE` and the gSOAP XML DOM API documentation.
*/
...
@see `#SOAP_DOM_ASIS`, `#SOAP_DOM_TREE` and the gSOAP XML DOM API documentation.
*/
...
@see `#SOAP_DOM_ASIS`, `#SOAP_DOM_NODE` and the gSOAP XML DOM API documentation.
*/
...
struct soap {
  ...
  */
  const char *actor;
  /// User-definable <i>`xml:lang`</i> attribute value of <i>`SOAP-ENV:Text`</i> to output the SOAP Fault string/reason (`en` by default)
  /**
  @see `::SOAP_ENV__Reason`.
  */
  const char *lang;
  /// User-definable maximum number of keep-alive message exchanges per connection (`#SOAP_MAXKEEPALIVE` by default)
  /**
  @par Example:
  ...
  */
  ...
  /// User-definable maximum number of active cookies allowed to be set with `::soap_set_cookie` before cookie memory is reused (32 by default)
  int cookie_max;
  ...
  */
  SOAP_SINK *os;
  /// User-definable maximum message length that is permitted to be received, zero means unlimited (2GB by default)
  /**
  @par Example:
  ...
  */
  ...
  /// User-definable timeout to send or receive an entire message, positive timeout values are seconds, negative timeout values are microseconds, zero means no timeout (0 by default)
  /**
  @par Example:
  ...
  */
  ...
  /// User-definable value to set `SO_RCVBUF` `setsockopt` (`#SOAP_BUFLEN` by default)
  /**
  @par Example:
  ...
  */
  int rcvbuf;
  /// User-definable value to set `SO_SNDBUF` `setsockopt` (`#SOAP_BUFLEN` by default)
  /**
  @par Example:
  ...
  */
  ...
  /// User-definable proxy port which should be set to connect through an HTTP proxy (8080 by default)
  /**
  @see `::soap::proxy_host`.
  */
  int proxy_port;
  ...
  */
  ...
  /// User-definable CORS Access-Control-Allow-Origin HTTP header string default value (`*` by default)
  /**
...
void soap_set_version(
    ...
    short version)     ///< SOAP version (0 = no SOAP, 1 = SOAP 1.1, 2 = SOAP 1.2)
  ;
...
if (soap_connect_command(soap, endpoint, NULL, SOAP_POST)
 || soap_send(soap, "<doc title=\"Example\">Some text</doc>\n")
 || soap_end_send(soap)
 || soap_begin_recv(soap)
 || (response = soap_get_http_body(soap, &response_len)) != NULL
...
/// SOAP Header structure generated by wsdl2h and soapcpp2
/**
This structure is generated by the wsdl2h tool from a WSDL with SOAP Header definitions and/or by soapcpp2 to complete the SOAP Header definitions.  A SOAP Header contains meta-data, such as WS-Addressing and WS-Security headers, associated with messages.  SOAP Header elements may be marked mustUnderstand with <i>`SOAP_ENV:mustUnderstand="1"`</i> to force the receiver to produce an error if the header element was not recognized.
...
struct SOAP_ENV__Header {
  mustUnderstand int *ns__someHeaderValue; // optional element with SOAP_ENV:mustUnderstand="1" attribute
};
...
/// SOAP Fault structure generated by soapcpp2
/**
This structure is generated by the wsdl2h tool from a WSDL with SOAP Fault definitions and/or by soapcpp2 to complete the SOAP Fault definitions.  A SOAP Fault contains error information specified by the `::SOAP_ENV__Fault::faultstring` (SOAP 1,1) or `::SOAP_ENV__Fault::SOAP_ENV__Reason` (SOAP 1.2).  The SOAP Fault detail `::SOAP_ENV__Fault::SOAP_ENV__Detail` may include specific elements related to the fault.
...
struct SOAP_ENV__Detail {
  ...
  int __type;
  void *fault;
  struct ns__someElement *ns__someElement;
};
...
/// SOAP Fault Code structure generated by soapcpp2
...
/// SOAP Fault Detail structure generated by soapcpp2
struct SOAP_ENV__Detail {
