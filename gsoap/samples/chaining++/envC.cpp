/* envC.cpp
   Generated by gSOAP 2.8.26 from env.h
...
SOAP_SOURCE_STAMP("@(#) envC.cpp ver 2.8.26 2015-11-29 18:43:08 GMT")
...
{	switch (p->type)
	...
	case SOAP_TYPE_h__authentication:
		if (p->size < 0)
			SOAP_DELETE(static_cast<struct h__authentication*>(p->ptr));
		else
			SOAP_DELETE_ARRAY(static_cast<struct h__authentication*>(p->ptr));
		break;
#ifndef WITH_NOGLOBAL
	case SOAP_TYPE_SOAP_ENV__Header:
		if (p->size < 0)
...
SOAP_FMAC3 void SOAP_FMAC4 env_finsert(struct soap *soap, int t, int tt, void *p, size_t index, const void *q, void **x)
{
	...
	{
	case SOAP_TYPE_h__authentication:
		DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Copy struct h__authentication type=%d location=%p object=%p\n", t, p, q));
...
{
	soap_default_h__authentication(NULL, this);
}
...
{
	if (a->userid)
		soap_set_attr(soap, "userid", (char*)a->userid, 1);
	if (a->passwd)
		soap_set_attr(soap, "passwd", (char*)a->passwd, 1);
	(void)soap; (void)tag; (void)id; (void)a; (void)type; /* appease -Wall -Werror */
...
SOAP_FMAC3 struct h__authentication * SOAP_FMAC4 soap_in_h__authentication(struct soap *soap, const char *tag, struct h__authentication *a, const char *type)
{
	if (soap_element_begin_in(soap, tag, 0, type))
		return NULL;
	a = (struct h__authentication *)soap_id_enter(soap, soap->id, a, SOAP_TYPE_h__authentication, sizeof(struct h__authentication), NULL, NULL, NULL, NULL);
	if (!a)
		return NULL;
	soap_default_h__authentication(soap, a);
	if (soap_s2string(soap, soap_attr_value(soap, "userid", 0), (char**)&a->userid, 0, -1))
		return NULL;
	if (soap_s2string(soap, soap_attr_value(soap, "passwd", 0), (char**)&a->passwd, 0, -1))
		return NULL;
	if (soap->body && !*soap->href)
	{
		for (;;)
	...
	{	a = (struct h__authentication *)soap_id_forward(soap, soap->href, (void*)a, 0, SOAP_TYPE_h__authentication, SOAP_TYPE_h__authentication, sizeof(struct h__authentication), 0, env_finsert, NULL);
		if (soap->body && soap_element_end_in(soap, tag))
			return NULL;
	}
	return a;
...
{
	DBGLOG(TEST, SOAP_MESSAGE(fdebug, "soap_instantiate_h__authentication(%p, %d, %s, %s)\n", soap, n, type?type:"", arrayType?arrayType:""));
	(void)type; (void)arrayType; /* appease -Wall -Werror */
	struct h__authentication *p;
	size_t k = sizeof(struct h__authentication);
	if (n < 0)
	{	p = SOAP_NEW(struct h__authentication);
	}
	else
	{	p = SOAP_NEW_ARRAY(struct h__authentication, n);
		k *= n;
	}
	DBGLOG(TEST, SOAP_MESSAGE(fdebug, "Instantiated struct h__authentication location=%p n=%d\n", p, n));
	soap_link(soap, p, SOAP_TYPE_h__authentication, n, env_fdelete);
	if (size)
		*size = k;
	return p;
}
...
{
	if (soap_out_h__authentication(soap, tag?tag:"h:authentication", -2, a, type))
		return soap->error;
	return soap_putindependent(soap);
}
...
{
	id = soap_element_id(soap, tag, id, *a, NULL, 0, type, SOAP_TYPE_h__authentication, NULL);
	if (id < 0)
		return soap->error;
...
SOAP_FMAC3 struct h__authentication ** SOAP_FMAC4 soap_in_PointerToh__authentication(struct soap *soap, const char *tag, struct h__authentication **a, const char *type)
{
	...
	*a = NULL;
	...
	{	a = (struct h__authentication **)soap_id_lookup(soap, soap->href, (void**)a, SOAP_TYPE_h__authentication, sizeof(struct h__authentication), 0, NULL);
		if (soap->body && soap_element_end_in(soap, tag))
			return NULL;
	}
	return a;
...
{
	if (soap_out_PointerToh__authentication(soap, tag?tag:"h:authentication", -2, a, type))
		return soap->error;
	return soap_putindependent(soap);
}
