...
static void gen_uri(struct soap *ctx, ULONG64 n);
         ...
         )
      {
        ...
        while (f && *++a)
        {
          switch (*a)
          {
            ...
            case 'h':
              fprintf(stderr,
                  "Usage: testmsgr [-aact] [-b] [-c] [-dnum] [-f] [-h] [-i] [-j] [-lmax] [-mmax] [-nlen] [-pperc] [-qperc] [-rseed] [-tsec] [-v] [-A] [-C] [-H] [-M] [-Pper] [-Rrep] [-Ssel] [infile|-] [URL|port]\n\n"
                  ...
                  "-Pper   set XML element permutation indicator tag (default=" PER ")\n"
...
static void gen_value(struct soap *ctx, const char *s)
{
  ...
  if (s[0] == '%' && s[1] == '[')
  {
    ...
    if (!strncmp(s, "%[[ENTITY", 9) || !strncmp(s, "%[[ID", 5) || !strncmp(s, "%[[IDREF", 8) || !strncmp(s, "%[[NAME", 7) || !strncmp(s, "%[[NCNAME", 9) || !strncmp(s, "%[[NMTOKEN", 10) || !strncmp(s, "%[[TOKEN", 8))
    {
      size_t i = s[4] == 'C' ? 9 : s[4] == 'M' ? 10 : s[4] == 'A' ? 7 : s[5] == 'R' ? 8 : s[3] == 'I' ? 5 : s[3] == 'T' ? 8 : 9;
      if (s[i] == '[')
      {
        unsigned long n, m;
...
static void gen_id(struct soap *ctx)
{
  ...
  static int id = 0;
  soap_send(ctx, "id");
  soap_send(ctx, soap_int2s(ctx, id++));
}
