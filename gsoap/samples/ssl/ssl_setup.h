/*
	...
	Set up ssl client context with soap_ssl_client_setup():
...
int soap_ssl_client_setup(
    ...
    const char *keyfile,  /* a keyfile is required only when client must authenticate to server, NULL otherwise */
    const char *password, /* password to read the key file (not used with GNUTLS), NULL otherwise */
    const char *cacert,   /* optional file name of CA PEM file, NULL to search CA stores */
    const char *capath);  /* optional path to CA PEM files, NULL to search CA stores */
