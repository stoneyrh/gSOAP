...
void soap_default_xsd__date(struct soap *soap, struct tm *a)
{ (void)soap; /* appease -Wall -Werror */
  memset((void*)a, 0, sizeof(struct tm));
}

void soap_serialize_xsd__date(struct soap *soap, struct tm const *a)
{ (void)soap; (void)a;
}

const char *soap_xsd__date2s(struct soap *soap, struct tm a)
{ strftime(soap->tmpbuf, sizeof(soap->tmpbuf), "%Y-%m-%dZ", &a);
  return soap->tmpbuf;
}

int soap_out_xsd__date(struct soap *soap, const char *tag, int id, const struct tm *a, const char *type)
...
int soap_s2xsd__date(struct soap *soap, const char *s, struct tm *a)
{ memset((void*)a, 0, sizeof(struct tm));
  if (s)
  { char *t;
    unsigned long d;
...
struct tm *soap_in_xsd__date(struct soap *soap, const char *tag, struct tm *a, const char *type)
{ if (soap_element_begin_in(soap, tag, 0, NULL))
    return NULL;
  if (*soap->type
   && soap_match_tag(soap, soap->type, type)
   && soap_match_tag(soap, soap->type, ":date"))
